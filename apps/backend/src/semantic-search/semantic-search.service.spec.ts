...
function makeCountry(overrides: Partial<{
  cca2: string; cca3: string; name: string; continent: string;
  subregion: string; flag: string; landlocked: boolean;
  languages: string[]; currencies: string[]; latlng: [number, number];
}> = {}) {
  return {
...
describe('SemanticSearchService', () => {
  describe('search — com resultados FTS', () => {
    it('retorna resultado com campos corretos', async () => {
      ...
      const svc = makeService({ countries: [country], ftsResults: [{ cca2: 'BR', bm25: 5 }] });
    ...
    it('score é bm25 + soma dos tag scores', async () => {
      const country = makeCountry();
      const svc = makeService({ countries: [country], ftsResults: [{ cca2: 'BR', bm25: 10 }] });
      const res = await svc.search('Brasil');
    ...
    it('ordena por score desc', async () => {
      ...
      const jp = makeCountry({ cca2: 'JP', name: 'Japan', continent: 'Asia', subregion: 'Eastern Asia', flag: 'https://flagcdn.com/jp.svg', languages: ['Japanese'], currencies: ['JPY'] });
      const svc = makeService({
        countries: [br, jp],
        ftsResults: [{ cca2: 'JP', bm25: 3 }, { cca2: 'BR', bm25: 8 }],
      });
    ...
    it('limita a 15 resultados', async () => {
      ...
      const ftsResults = countries.map((c, i) => ({ cca2: c.cca2, bm25: 20 - i }));
